...
class MailResetPasswordNotification extends Notification
{
    ...
    public function toMail($notifiable)
    {
        if(\Request::route()->getName() == "admin.password.email"){
            $url = url(config('app.url').route('admin.password.reset', ['token' => $this->token, 'email' => $notifiable->getEmailForPasswordReset()], false));
        }else{
        ...
        return (new MailMessage)
            ...
            ->action(Lang::getFromJson('Reset Password'), $url/*url(config('app.url').route('password.reset', ['token' => $this->token, 'email' => $notifiable->getEmailForPasswordReset()], false))*/)
